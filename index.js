...
client.connect((err) => {
  ...
  app.get("/checkedItem/:id", (req, res) => {
    ...
    foodCollection.find({ _id: id }).toArray((err, documents) => {
  ...
  app.post("/orderedInfo", (req, res) => {
    ...
    orderedCollection.insertOne(ordered).then((result) => {
      res.send(result.insertedCount > 0);
    });
  });
  app.get("/orders", (req, res) => {
    const queryEmail = req.query.email;
    orderedCollection.find({ email: queryEmail }).toArray((err, documents) => {
      res.send(documents);
    });
  });
